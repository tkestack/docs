...
仔细检查每个节点的硬件和软件需求：[installation requirements](./部署环境要求.md)
...
- **GPU 类型**：（按需使用，可直接选择【**不设置**】）
  ...
  - **Virtual**：平台会自动为集群安装 [GPUManager](./../产品特色功能/GPUManager.md)  扩展组件
...
安装失败请首先检查硬件和软件需求：[installation requirements](./部署环境要求.md)

<!-- 404 -->
可参考[FAQ installation](./../FAQ/部署类)获得更多帮助。
